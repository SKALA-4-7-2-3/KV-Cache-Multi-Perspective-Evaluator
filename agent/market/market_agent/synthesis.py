...
def retain_drafts(data, draft, packet, evidence, level, issues, previous=(), *, sources=None):
    """Keep rejected model wording for attributed review, never as accepted evidence."""
    retained={row['id']:dict(row) for row in previous}
    for row in draft.assessments:
        scoped=[issue for issue in issues if issue.get('scope')=='global' or
            (issue.get('tech_id'),issue.get('criterion_id'))==(row.tech_id,row.criterion_id)]
        if not scoped or row.tech_id not in data.technologies:
            continue
        supports=[]
        for key in dict.fromkeys(row.quote_ids):
            item=packet.get(key)
            if not item or not packet_item_valid(item,evidence,data,2,sources):
                continue
            source=evidence[item['evidence_id']]
            if row.tech_id not in source.tech_ids:
                continue
            supports.append({'evidence_id':source.id,'quote':item['text'],
                'locator':item['locator'],'access_status':source.access_status})
        if not supports or not (row.observation.strip() or row.judgment.strip()):
            continue
        identity=json.dumps(row.model_dump(mode='json'),ensure_ascii=False,sort_keys=True)
        key='MARKET-SYNTHESIS-DRAFT-'+hashlib.sha256(identity.encode()).hexdigest()[:16]
        notes=[issue['code']+(': '+issue['reason'] if issue.get('reason') else '') for issue in scoped]
        notes=['출처와 연결된 모델 초안이며 검토에서 승인된 시장 사실이 아닙니다.',*notes]
        old=retained.get(key,{})
        retained[key]={
            'id':key,'technology_ids':[row.tech_id],'criterion_id':row.criterion_id,
            'text':'\n'.join(value for value in [row.observation,row.judgment] if value.strip()),
            'observation':row.observation,'judgment':row.judgment,'basis':'inference',
            'source_scope':row.relation_to_technology,'conditions':list(row.conditions),
            'limitations':list(row.limitations),'evidence_ids':list(dict.fromkeys(s['evidence_id'] for s in supports)),
            'supports':supports,'review_status':'needs_review','generation_method':'model_synthesis',
            'evaluation_level':level,
            'review_notes':list(dict.fromkeys([*old.get('review_notes',[]),*notes])),
        }
