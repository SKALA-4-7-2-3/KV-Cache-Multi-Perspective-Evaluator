PROMPT_VERSION = 'market-v11-adaptive-synthesis-repair'
...
SYNTHESIS_PROMPT = COMMON + """
원자료를 종합해 targets에 지정된 기술·시장 항목을 각각 독립적으로 평가한다.
현재 research_policy가 허용하는 자료 유형과 관계 범위를 따른다. 다음 단계의 기준을 미리 적용하지 않는다.
관찰(observation)과 판단(judgment)을 나눈다. 관찰에는 선택한 원자료가 실제 설명한 대상·조건을 적고,
판단에는 그 관찰이 선정 기술의 시장성에 주는 의미와 도입 조건을 구체적으로 연결한다.
자료에 있는 표, 짧은 항목, 긴 문단, 키워드의 동의 표현도 의미로 해석한다. 문장 형식만으로 버리지 않는다.
quote_ids는 packet의 MAT-... 키 중 실제 사용한 조각만 선택한다. 다른 기술 소유 자료를 사용하지 않는다.
수치의 단위·연도·지역·정의가 일부 없으면 그 공백을 limitations에 적고 원문 범위의 수치를 관찰로 보존할 수 있다.
누락된 값을 추정하거나 실제치/전망치, 생산량/생산능력/매출을 바꾸지 않는다.
주변 지식으로 고객명·출시·지원 버전·시장 수치·성능을 덧붙이지 않는다. quote_ids의 내용에 있는 숫자만 쓴다.
같은 기술군의 출시·채택은 그 제품의 관찰로 명시하고, 선정 기술에 대해서는 도입 경로·장벽·조건을 추론한다.
관련 자료만으로 선정 논문이 출시되었거나 고객이 사용 중이라고 쓰지 않는다. 연구 실험은 실고객 성과가 아니다.
비교 제약(comparison_constraints)을 준수한다. 실측과 시뮬레이션 배수를 직접 대결시키지 않는다.
검색 발췌는 원문 미확보, 기술 요약은 독립 검증 미실시 조건을 붙인다.
자료가 부족하더라도 실제 확인한 관찰에서 도출할 수 있는 제한된 판단을 작성한다.
연결할 자료가 전혀 없거나 허용 관계 밖이면 그 행은 assessments에 넣지 않는다. 정형 문구로 채우지 않는다.
조건(conditions)과 한계(limitations)를 구체적으로 적는다. 기존 평가를 반복하지 말고 이번 targets만 작성한다.
relation_to_technology는 결론의 대상이 아니라 선택한 근거의 관계다. 일반 CXL 제품·규격은 Photonic-CXL에 method_family,
다른 KV 양자화 구현·vLLM 기능은 RDKV에 method_family, 전체 AI 인프라 시장은 adjacent다.
여러 범위가 섞이면 가장 넓은 관계를 선택한다. 논문 제목 조각 하나를 끼워 넣어 exact로 만들지 않는다.
논문 제목·메뉴만 있는 조각은 신원 확인용이다. 그것으로 제품화·채택·지원·표준화의 부재를 결론내릴 수 없다.
검색한 일부 자료에 없다는 관찰은 해당 자료 범위로 한정하고, 판단에는 실제 자료가 제시한 구현 경로·고객 후보·통합 조건을 쓴다.
예: '정식 표준이 없다' 대신 '관련 규격의 기능을 도입 기준으로 삼되 선정 구현의 적합성 시험을 요구한다'.
성능 수치마다 원래 기술명·측정 환경·기준선을 보존한다. 다른 CXL 데모의 처리량·TCO 수치를 Photonic-CXL 성과에 합치지 않는다.
조각에 실제 시장 규모·생산량·성장 수치가 있다면 부족한 지역·정의를 한계로 붙여 활용한다. 기술 효과 설명만으로 대체하지 않는다.
issues의 missing_material_reference는 현재 packet의 ID를 다시 선택해 수정한다.
identity_requires_related_scope는 관련 근거에 method_family 또는 adjacent를 사용해 수정한다.
unsupported_number는 선택한 조각에 없는 수치를 제거하거나 해당 수치가 있는 조각을 선택해 수정한다.
semantic_review_rejected는 검토 reason에 맞춰 관찰의 대상·범위·조건을 고친다.
"""

SYNTHESIS_REVIEW_PROMPT = COMMON + """
제출된 관찰·평가를 별도로 검토한다. 앞 단계 판단을 신뢰하지 말고 연결된 원자료와 대조한다.
모든 제출 행마다 supported, relevant, scope_preserved, uncertainty_preserved와 reason을 반환한다.
supported: 관찰의 수치·대상·조건이 원자료에 있고 판단의 추론 경로가 설명되는가.
relevant: 실제 의미상 해당 시장 항목을 설명하는가. 특정 키워드가 없다는 이유로 거부하지 않는다.
scope_preserved: 현재 research_policy의 관계 범위를 지키며 다른 제품의 출시·채택·규격을 선정 기술 실적으로 바꾸지 않았는가.
uncertainty_preserved: 공급사 주장, 연구/시뮬레이션, 기술 요약, 검색 발췌의 한계와 원문에 없는 시장 정의·연도·지역을 정직하게 표시했는가.
직접 실적이 부족해도 현재 단계가 허용하면 같은 기술군/인접시장 자료에 근거한 조건부 판단을 인정한다.
그럴듯한 일반론만 있고 자료와 결론의 연결이 없으면 relevant=false다.
표의 열을 잘못 연결한 수치, 출처 없는 고객명, 불가능한 성능 직접 비교는 supported=false다.
진위를 외부에서 새로 검증했다는 뜻은 아니다. 제공된 자료의 범위와 해석만 검토한다.
다음 오류는 반드시 거부한다: 제목만으로 미상용화·미채택·지원 미흡·표준 부재를 단정하는 경우,
자료의 침묵을 실제 부재로 바꾸는 경우, 서로 다른 업체/데모의 성능 수치를 선정 논문의 성과로 합치는 경우.
예: Photonic-CXL 논문의 6.6배와 다른 CXL 데모의 21.9배·TCO 수치는 같은 성능 묶음이 아니다.
관련 CXL 규격/제품 자료를 exact라고 하거나 관련 양자화 기능을 RDKV 자체 지원으로 쓰면 scope_preserved=false다.
관찰에는 원자료 주체가 드러나야 하며, 공급사의 수요 전망은 공급사 주장으로 한정해야 한다.
시장 규모 수치가 없어도 적용 가능한 고객군과 수요 요인에 대한 조건부 평가는 허용한다. 근거 없이 수요가 크게 성장한다고 단정하면 거부한다.
실제 도입을 확인하지 못해도 근거가 설명하는 기능과 도입 조건을 구체적으로 연결한 판단은 허용한다.
"""
