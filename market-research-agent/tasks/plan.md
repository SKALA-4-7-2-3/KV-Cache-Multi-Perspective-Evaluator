# 시장조사 에이전트 수정 계획 v1.1

> 최신 설계: **2026-09-22 JSON 라이브 실행의 v1.2 구현 및 JSON 4파일 입력 지원을 반영하고 실제 결과를 재검증했다.** [상세 설계](../docs/MARKET_JSON_RELIABILITY_DESIGN.md)와 [작업 목록 J1~J8](todo.md)을 따른다. 아래 v1.0/v1.1 완료 기록은 당시 검증 범위의 이력이다.
...
## 10. 후속 개선 v1.1 — 근거 추출과 평가 분리

- 계획일·구현일: 2026-09-22. 상태: 구현·검증 완료. 자동 실행 후 원문 대조 검토본을 별도 저장.
- 대상: `feat/market-agent`의 `market-research-agent/`.
- 재현 실행: `20260922_live_connected`. 원문·모델 응답은 이 실행의 로컬 내부 캐시에 보존되어 있으며 Git에는 포함하지 않는다.
- 유지할 외부 계약: 동일 Markdown 입력 형식, SW/HW 각 6개 시장성 항목, 인용 근거, 전달 MD 한 개. 상세 진단은 내부 기록으로 분리한다.
- 목적: 수집된 유효 근거를 해당 범위에 맞게 보존하고, 실제 근거 공백과 처리 실패를 구분한다. 긍정 판정이나 미확인 감소 개수를 성공 목표로 삼지 않는다.

### 10.1 관찰 결과와 원인

| 관찰 사실 | 구현상 원인 또는 한계 |
| --- | --- |
| 검색 4회, 원문 조회 9회, LLM 2회가 성공 | API 연결 성공은 내용의 타당성을 보장하지 않음 |
| 두 논문 페이지에 제목·메타데이터·메뉴가 주로 남고 초록이 없음 | 추출 성공만으로 full_text 접근 상태를 부여하며 본문 품질 검사가 없음 |
| 최종 모델 응답의 근거 성격은 fact 5개, unknown 7개 | fact 5개가 입력 요약을 시장 근거로 사용하여 검증에서 제외됨. 모델 최종 도입 판정도 이미 12개 모두 unknown이므로 검증기만이 모든 판정을 바꾼 것은 아님 |
| SW 인용에 다른 연구 문구가 논문 원문 ID로 연결됨 | 자료를 직접 12개 평가로 한꺼번에 작성하는 과정에서 주장·출처가 혼합됨 |
| Marvell 관련 제품 설명이 모델에 전달됐지만 두 회차의 context_findings가 모두 비어 있음 | 관련 정보 보존을 모델의 자발적인 출력에 의존함 |
| SW 보완 질의가 질문형 문장이고 무관한 검색 결과가 반환됨 | 제안 질의를 키워드·대상·범위에 맞게 검증하지 않음. 한국어 자체를 오류로 취급하지 않음 |
| 남은 호출량이 있어도 동일 오류를 남긴 채 종료 | 보완 1회 안에서 재검색과 인용 수정의 필요를 구분하지 못함 |

### 10.2 제안하는 처리 흐름

`입력 → 검색 → 본문 품질 검사 → 근거 항목 추출 → 항목별 근거 검증 → 시장 평가 작성 → 결과 검사 → 필요한 단계만 보완 → MD 출력`

새 독립 에이전트나 프레임워크를 추가하지 않고 기존 시장 역할의 LangGraph 노드를 분리한다. 내부 근거 항목은 대상, 주장, 출처 ID, 짧은 원문 인용, 위치, 날짜·범위, 선정 기술과의 관계, 적용 가능한 평가 항목을 가진다. 보고서에는 기존 시장성 표와 인용만 출력한다.

### 10.3 수정 원칙

**A. 접근 성공과 본문 품질 분리**

- Evidence에 내용 상태를 별도로 기록한다. 본문 확인, 메타데이터만 확보, 접근 실패, 품질 판단 보류를 구분한다. 기존 full_text 접근 표시는 그대로 두더라도 내용 검사 통과와 같은 뜻으로 사용하지 않는다.
- 제목·링크·메뉴를 제외한 내용, 문서 유형과 제목·논문 식별자 일치를 검사한다. 글자 수 하나만으로 충분한 근거라고 인정하지 않는다.
- 메타데이터뿐이면 같은 논문·버전의 공개 HTML/PDF 등 대체 경로를 최대 1회 시도한다. 예산·공개 URL 검사·중복 방지를 그대로 적용한다. 다른 연구 자료를 해당 논문의 원문으로 대체하지 않는다.
- 본문 품질을 통과해도 초록만으로 제품 판매·고객 도입을 입증하지 못한다. 접근 범위와 주장 적합성은 별도 검사다.

**B. 검색어는 기술 자체와 기술군을 나누어 구성**

- LLM은 부족한 항목·우선순위를 제안하고 코드는 검증된 기술명·별칭·기술군과 항목별 키워드로 검색어를 구성한다. 임의 질문 문장을 그대로 실행하지 않는다.
- 예시: 선정 구현은 `"RDKV" "KV cache" implementation license`, 기술군은 `"KV cache" quantization inference serving cost`로 범위를 분리한다. 예시는 실제 조사 결과가 아니다.
- 논문명만으로 전체 시장 규모를 검색하지 않는다. 관련 기술군 수치는 시장 정의·지역·연도·단위·전망 여부와 함께 별도로 취급하며 선정 기술 수치로 바꾸지 않는다.
- 검색 실패·무관한 결과를 기록하고, 중복 질의나 언어만을 이유로 한 무조건 제외를 피한다.

**C. 근거 추출과 평가 작성 분리**

- 첫 LLM 단계는 품질을 통과한 원문에서 주장을 작은 단위로 추출한다. 입력의 provided_summary는 배경 자료이며 웹 근거와 섞어 사실로 만들지 않는다.
- 코드가 주장별 인용 문자열·출처 ID·원문 위치·시점·대상 연결을 검사한다. 문자열 일치가 의미상의 입증을 보장하지 않는다는 한계를 유지한다.
- 다음 평가 단계는 검증된 근거 ID를 참조해 12개 항목을 작성한다. 새 URL·인용 문구를 자유롭게 재생성하지 못하게 한다.
- 입력 요약은 기존 허용 범위인 business_value의 조건부 추론에만 사용하며, 고객·가격·절감률을 만들지 않는다.
- 잘못된 인용이 섞이면 독립적으로 유효한 근거를 보존한다. 인용을 삭제한 채 원래 결론을 유지하는 자동 처리는 금지하고, 결론은 남은 근거로 다시 작성·검사한다.

**D. 관련 정보의 누락 방지**

- 선정 논문 자체의 판정과 관련 기술군·제품 정보는 별도로 유지한다. 관련 제품의 발표는 선정 기술 상용화와 동일하지 않다.
- 검증한 근거 풀과 보고서에 사용된 근거를 비교하여 누락·제외 이유를 내부에 기록한다. 관련 정보는 검증한 문장을 기준으로 context_findings에 연결하며, 모델이 빈 배열을 반환했다는 이유만으로 조용히 사라지지 않게 한다.
- 관련성이 낮거나 중복인 정보는 제외할 수 있다. 모든 수집 문서를 보고서에 강제로 넣거나 관련 정보 최소 개수를 채우지 않는다.
- 회차 간 유효 근거를 ID별로 유지한다. 반대 근거·잘못된 연결이 확인되면 취소·수정 이유를 남긴다.

**E. 오류별 보완과 실행 한도**

| 문제 | 필요한 보완 |
| --- | --- |
| 메타데이터만 확보 | 같은 문서 대체 경로 조회 |
| 근거 자체가 부족 | 미해결 항목을 위한 추가 검색 |
| 인용 ID·문구가 잘못됨 | 보유 원문으로 근거 추출 또는 해당 평가만 수정 |
| 검증한 관련 정보 누락 | 기존 근거를 다시 연결하고 결과 재검사 |
| 제한 안에서 해결 불가 | 구체적 이유를 남기고 종료 |

- 입력 상한 검색 6 / 원문 10 / LLM 5를 유지한다. 재시도·대체 URL 조회도 같은 예산에 포함한다.
- 기본 LLM 호출은 근거 추출 1회 + 평가 작성 1회다. 논리적 보완은 전체 흐름에서 최대 1회, 필요한 LLM 단계만 추가 1회 실행한다. 전송 재시도를 포함해 LLM 5회를 넘지 않는다. 한도가 더 작으면 가능한 단계와 미완료 사유만 반환한다.
- 보완 1회의 위치를 오류가 발생한 단계에 맞춘다. 이미 교정 기회를 사용했으면 다른 단계에서 다시 보완을 시작하지 않는다. 남은 예산을 소진하는 것이 목표가 아니다.
- 부모가 보완을 관리하면 기존 auto_repair=False와 역할별 Budget 계약을 유지한다.
- 원문을 모델에 전달한 상태와 실제 근거를 검증한 상태를 내부에서 구분한다. 키워드가 포함됐다는 이유만으로 항목 검토가 끝났다고 기록하지 않는다.

### 10.4 검증과 완료 조건

1. 저장된 실패 사례로 먼저 검증한다. 메타데이터뿐인 논문, 다른 연구의 인용 혼입, 요약+웹 혼합, 빈 관련 정보, 부적절한 보완 질의를 작은 회귀 사례로 고정한다. 테스트 자료에 키·전체 캐시를 넣지 않는다.
2. 합성 응답을 이용한 연결 검사는 유효한 관련 제품 정보가 한 항목으로 실제 출력되고, 선정 논문은 미확인으로 남는 경우를 검증한다. 무조건 긍정 또는 근거 개수 채우기는 실패다.
3. 코드 변경 후 기존 58개 검사와 새 회귀 검사, compileall, MD 한 개·12개 평가·근거 무결성·한도 검사를 수행한다.
4. 저장 원문을 사용해 새 처리 흐름을 검증한다. 과거 모델 응답만 다시 검사한 경우와 새로운 모델 호출을 한 경우를 구분한다.
5. 위 검사가 통과하면 같은 입력·기준일·모델·호출 상한으로 새 라이브 실행 1회를 하고 주요 인용과 결론을 사람이 대조한다. 미확인 개수를 줄이기 위한 반복 호출은 하지 않는다.
6. 실행 성공과 보고서 품질을 별도로 보고한다. 유효한 근거가 보고서에서 이유 없이 누락되지 않고, 처리 실패가 단순 자료 부족으로 설명되지 않아야 완료다. 모든 시장 항목을 확인했다고 보장하지 않는다.

### 10.5 위험·호환성과 작업 순서

- 주장별 분리는 모델 호출 구조와 내부 캐시 형식에 영향을 준다. 내부 스키마·프롬프트 버전과 코드 fingerprint를 갱신하고 구형 캐시는 새 성공 결과로 자동 재사용하지 않는다.
- 시장 보고서와 입력의 외부 MD 계약은 유지한다. 원본 입력·과거 전달본·기존 라이브 기록은 덮어쓰지 않는다.
- 근거 사용 오류를 없애기 위해 검증 규칙을 완화하거나, URL·출처 이름만으로 exact를 인정하지 않는다.
- 라이브 품질 실패를 해결했다고 주장하기 전에 실제 보고서를 대조한다. 58개 단위 검사 통과만으로 의미상의 정확성을 주장하지 않는다.
- 순서: **P0 재현 → P1 본문 품질 → P2 검색어 → P3 근거 계약 → P4 추출·평가 분리 → P5 보존·보완 → P6 실제 검증**. 상세 완료 조건은 todo.md에 기록한다.

### 10.6 실제 실행으로 보완한 설계

원문 구절 ID를 코드가 발급하고 모델이 선택하도록 변경했다. 25단어 창은 문장 의미가 잘려 최대 100단어의 완전한 문장으로 교체했으며, 외부 보고서 발췌는 URL당 25단어 제한을 유지한다. 작성 모델은 모든 Claim의 의미 지지 여부를 검토한다. 검토 누락·거부는 자동 제외하고, 보고서에는 통과한 문장과 별도 출처 귀속만 사용한다. 직접 근거 없는 행의 unknown은 코드가 결정한다. 전송 재시도를 제외한 논리적 단계 수와 전체 호출 상한은 유지했다. 전체 시장 자료의 완전성이나 의미 검토 모델의 무오류를 보장하지 않는다.


## 11. JSON 입력 전환 — 2026-09-22

사용자가 제공한 논문별 paper_analysis JSON 1.1.0을 직접 입력으로 받도록 변경한다. 현재 분석 본문, evidence_registry, paper 및 nullable 필드를 기준으로 검증·정규화하며 raw JSON의 quality/run 등은 기록으로 보존한다. 상위 파일에 없는 시장 실행 설정은 CLI 옵션으로 분리한다. 두 논문은 기술당 6개 항목을 생성하고, 단일 논문도 처리한다. 제공된 발췌를 시장 검증 완료 원문으로 승격하지 않는다.

실제 파일 파싱 → 새 실패/참조 테스트 → CLI·캐시·기술 배경 연결 → 기존 회귀와 가상 제공자 전체 실행 → 문서·커밋·푸시 순서로 검증했다. 상세 완료 결과는 docs/JSON_INPUT.md에 있다.


## JSON 라이브 실패 개선 v1.2 — 2026-09-22 구현 반영

[원인 분석 및 상세 수정 설계](../docs/MARKET_JSON_RELIABILITY_DESIGN.md)를 최신 구현 기준으로 반영했다. 아래는 최초 계획이며 구체적 변경·실제 실행 결과는 상세 설계 8절과 LIVE_VALIDATION.md에 기록한다.

- 관찰: 잘못된 상위 인용 ID, 관련 제품의 exact 오분류, 공용 보완 횟수에 의한 조사 중단, 오류 영향 범위 전파를 저장 응답 재생으로 확인.
- 변경 방향: 원본과 모델 배경 분리, 허용 인용 선택 계약, 대상·관계 재검토, 조사표·오류 범위 기록, 목적별 보완과 총예산 제어, 주장 의미·실증 수준 검토.
- 유지: JSON 1.1.0 및 기존 MD 입력, 기술당 시장 6개 항목, 단일 전달 MD, 기본 6/10/5 호출 상한.
- 명시적 정책 변경 제안: 기존 전체 보완 1회를 단계별 최대 1회로 분리하고, 초기 2/보완 최대 4질의, 내부 0.4 실행 상태와 CLI 부분 완료 코드를 도입.
- 순서: J1~J3 근거 경계 → J4~J5 조사·오류 상태 → J6 의미 검토 → J7a/J7b 계약·MD → J8 실제 검증. 상세 수용 기준은 todo.md에 미완료 체크로 기록.
- 이번 설계 검증: 추가 API 호출 0, 저장된 최초·보완 응답의 현재 검증기 재생과 오류 영향 범위 대조. 새 코드의 테스트·라이브 검증은 J8까지의 구현 후 수행한다.


## 12. 현재 수용 조건과 산출물

사용자가 지정한 dossier 두 개·evidence_registry·comparison의 4파일 입력으로 실행한다. 입력 참조·해시 검사를 통과한 뒤 웹 조사, 구절 선택, 의미 검토, 최종 audit를 수행한다. 자료에 없는 시장 수치를 채워 넣는 것은 목표가 아니다. 통합용 결과는 12개 시장 항목과 실제 사용한 인용을 갖는 자동 생성 MD 한 개다.

이번 라이브에서 반복적으로 확인한 오류에 한해서 검색·분류·검토를 수정하고 회귀 및 실제 출력으로 재확인한다. 전체 부모 Graph 연결은 이 작업에 포함하지 않는다. 원문 접근 실패 및 직접 시장 자료 부족은 최종 결과와 검증 기록에 공개한다.
